...
@section('title', 'Form Pengajuan PVT')
...
<div class="max-w-6xl mx-auto py-8 px-4">
    <div class="bg-white rounded-lg shadow-md p-8">
        ...
        <div class="mb-6">
            ...
            <h2 class="text-2xl font-bold text-gray-800">Form Pengajuan PVT</h2>
            <p class="text-gray-600 mt-2">Isi formulir di bawah ini untuk mengajukan PVT</p>
        </div>
        ...
        <form action="{{ route('pengajuan.storeDataForm') }}" method="POST" enctype="multipart/form-data" id="multiStepForm">
            @csrf
            <input type="hidden" name="jenis_ki" value="pvt">

            <!-- PAGE 1: Data Form -->
            <div id="page-1" class="form-page">
                <h3 class="text-xl font-bold text-gray-800 mb-6">Informasi PVT</h3>
                
                <!-- Judul PVT -->
                <div class="mb-4">
                    <label class="block text-gray-700 font-bold mb-2">Judul PVT *</label>
                    <input type="text" name="judul" id="judul" required
                        ...
                        placeholder="Masukkan judul PVT">
                </div>

                <!-- Nama Umum PVT -->
                <div class="mb-4">
                    <label class="block text-gray-700 font-bold mb-2">Nama Umum PVT *</label>
                    <textarea name="nama_umum_pvt" id="nama_umum_pvt" required rows="5"
                        class="border border-gray-300 rounded w-full py-2 px-3 text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-500"
                        placeholder="Masukkan nama umum PVT Anda"></textarea>
                </div>

                <!-- Nama Usulan PVT -->
                <div class="mb-4">
                    <label class="block text-gray-700 font-bold mb-2">Nama Usulan PVT *</label>
                    <textarea name="nama_usulan_pvt" id="nama_usulan_pvt" required rows="5"
                        class="border border-gray-300 rounded w-full py-2 px-3 text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-500"
                        placeholder="Masukkan nama usulan PVT Anda"></textarea>
                </div>

                <!-- Negara Asal PVT -->
                <div class="mb-4">
                    <label class="block text-gray-700 font-bold mb-2">Negara Asal PVT *</label>
                    <input type="text" name="negara_asal_pvt" id="negara_asal_pvt" required
                        class="border border-gray-300 rounded w-full py-2 px-3 text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-500"
                        placeholder="Masukkan negara asal PVT Anda">
                </div>

                <!-- Informasi Teknis -->
                <div class="mb-4">
                    <label class="block text-gray-700 font-bold mb-2">Informasi Teknis *</label>
                    <textarea name="informasi_teknis" id="informasi_teknis" required rows="5"
                        class="border border-gray-300 rounded w-full py-2 px-3 text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-500"
                        placeholder="Masukkan informasi teknis PVT Anda"></textarea>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="deskripsi" id="deskripsi" required rows="5"
                        class="border border-gray-300 rounded w-full py-2 px-3 text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-500"
                        placeholder="Jelaskan PVT Anda"></textarea>
                </div>

                <!-- Tanggal PVT -->
                <div class="mb-6">
                    <label class="block text-gray-700 font-bold mb-2">Tanggal PVT *</label>
                    <input type="date" name="tanggal_pvt" id="tanggal_pvt" required
                        class="border border-gray-300 rounded w-full py-2 px-3 text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-500"
                        max="{{ date('Y-m-d') }}">
                </div>
...
function validatePage(page) {
    if (page === 1) {
        // Validate page 1 fields untuk PVT
        const judul = document.getElementById('judul').value.trim();
        const namaUmumPVT = document.getElementById('nama_umum_pvt').value.trim();
        const namaUsulanPVT = document.getElementById('nama_usulan_pvt').value.trim(); // ← tambah
        const negaraAsalPVT = document.getElementById('negara_asal_pvt').value.trim(); // ← tambah
        const informasiTeknis = document.getElementById('informasi_teknis').value.trim(); // ← tambah
        const deskripsi = document.getElementById('deskripsi').value.trim();
        const tanggalPVT = document.getElementById('tanggal_pvt').value;
                
        if (!judul) {
            alert('Judul PVT harus diisi');
            document.getElementById('judul').focus();
            return false;
        }
        
        if (!namaUmumPVT) {
            alert('Nama Umum PVT harus diisi');
            document.getElementById('nama_umum_pvt').focus();
            return false;
        }

        if (!namaUsulanPVT) {
            alert('Nama Usulan PVT harus diisi');
            document.getElementById('nama_usulan_pvt').focus();
            return false;
        }

        if (!negaraAsalPVT) {
            alert('Negara Asal PVT harus diisi');
            document.getElementById('negara_asal_pvt').focus();
            return false;
        }
        
        if (!informasiTeknis) {
            alert('Informasi Teknis harus diisi');
            document.getElementById('informasi_teknis').focus();
            return false;
        }
        ...
        if (!tanggalPVT) {
            alert('Tanggal PVT harus diisi');
            document.getElementById('tanggal_pvt').focus();
            return false;
        }
