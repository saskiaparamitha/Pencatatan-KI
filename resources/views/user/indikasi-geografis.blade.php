...
@section('title', 'Form Pengajuan Indikasi Geografis')
...
<div class="max-w-4xl mx-auto py-8 px-4">
    <div class="bg-white rounded-lg shadow-md p-8">
        ...
        <div class="mb-6">
            ...
            <h2 class="text-2xl font-bold text-gray-800">Form Pengajuan Indikasi Geografis</h2>
            <p class="text-gray-600 mt-2">Isi formulir di bawah ini untuk mengajukan indikasi geografis</p>
        </div>
        ...
        <form action="{{ route('pengajuan.storeDataForm') }}" method="POST" enctype="multipart/form-data" id="multiStepForm">
            @csrf
            <input type="hidden" name="jenis_ki" value="indikasi_geografis">

            <!-- PAGE 1: Data Form -->
            <div id="page-1" class="form-page">
                <h3 class="text-xl font-bold text-gray-800 mb-6">Informasi Indikasi Geografis</h3>
                
                <!-- Judul/Nama Indikasi Geografis -->
                <div class="mb-4">
                    <label class="block text-gray-700 font-bold mb-2">Judul/Nama Indikasi Geografis *</label>
                    <input type="text" name="judul" id="judul" required
                        ...
                        placeholder="Masukkan judul indikasi geografis">
                </div>

                <!-- Nama Barang Indikasi Geografis -->
                <div class="mb-4">
                    <label class="block text-gray-700 font-bold mb-2">Nama Barang Indikasi Geografis *</label>
                    <textarea name="nama_barang_indikasi_geografis" id="nama_barang_indikasi_geografis" required rows="5"
                        class="border border-gray-300 rounded w-full py-2 px-3 text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-500"
                        placeholder="Masukkan nama barang indikasi geografis"></textarea>
                </div>

                <!-- Kualitas Indikasi Geografis -->
                <div class="mb-4">
                    <label class="block text-gray-700 font-bold mb-2">Kualitas Indikasi Geografis *</label>
                    <textarea name="kualitas_indikasi_geografis" id="kualitas_indikasi_geografis" required rows="5"
                        class="border border-gray-300 rounded w-full py-2 px-3 text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-500"
                        placeholder="Masukkan kualitas indikasi geografis"></textarea>
                </div>

                <!-- Karakteristik Indikasi Geografis -->
                <div class="mb-4">
                    <label class="block text-gray-700 font-bold mb-2">Karakteristik Indikasi Geografis *</label>
                    <textarea name="karakteristik_indikasi_geografis" id="karakteristik_indikasi_geografis" required rows="5"
                        class="border border-gray-300 rounded w-full py-2 px-3 text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-500"
                        placeholder="Masukkan karakteristik indikasi geografis"></textarea>
                </div>

                <!-- Kelas Nice Indikasi Geografis -->
                <div class="mb-4">
                    <label class="block text-gray-700 font-bold mb-2">Kelas Nice Indikasi Geografis *</label>
                    <textarea name="kelas_nice_indikasi_geografis" id="kelas_nice_indikasi_geografis" required rows="5"
                        class="border border-gray-300 rounded w-full py-2 px-3 text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-500"
                        placeholder="Masukkan kelas nice indikasi geografis"></textarea>
                </div>

                <!-- Sejarah -->
                <div class="mb-4">
                    <label class="block text-gray-700 font-bold mb-2">Sejarah *</label>
                    <textarea name="sejarah" id="sejarah" required rows="5"
                        class="border border-gray-300 rounded w-full py-2 px-3 text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-500"
                        placeholder="Masukkan sejarah indikasi geografis"></textarea>
                </div>

                <!-- Tradisi -->
                <div class="mb-4">
                    <label class="block text-gray-700 font-bold mb-2">Tradisi *</label>
                    <textarea name="tradisi" id="tradisi" required rows="5"
                        class="border border-gray-300 rounded w-full py-2 px-3 text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-500"
                        placeholder="Masukkan tradisi indikasi geografis"></textarea>
                </div>

                <!-- Jumlah Lisensi Indikasi Geografis -->
                <div class="mb-4">
                    <label class="block text-gray-700 font-bold mb-2">Jumlah Lisensi Indikasi Geografis *</label>
                    <textarea name="jumlah_lisensi_indikasi_geografis" id="jumlah_lisensi_indikasi_geografis" required rows="5"
                        class="border border-gray-300 rounded w-full py-2 px-3 text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-500"
                        placeholder="Masukkan jumlah lisensi indikasi geografis"></textarea>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="deskripsi" id="deskripsi" required rows="5"
                        class="border border-gray-300 rounded w-full py-2 px-3 text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-500"
                        placeholder="Jelaskan hak cipta Anda"></textarea>
                </div>

                <!-- Tanggal Indikasi Geografis -->
                <div class="mb-6">
                    <label class="block text-gray-700 font-bold mb-2">Tanggal Indikasi Geografis *</label>
                    <input type="date" name="tanggal_indikasi_geografis" id="tanggal_indikasi_geografis" required
                        class="border border-gray-300 rounded w-full py-2 px-3 text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-500"
                        max="{{ date('Y-m-d') }}">
                </div>
...
function validatePage(page) {
    if (page === 1) {
        ...
        const namaBarang = document.getElementById('nama_barang_indikasi_geografis').value.trim();
        const kualitas = document.getElementById('kualitas_indikasi_geografis').value.trim();
        const karakteristik = document.getElementById('karakteristik_indikasi_geografis').value.trim();
        const kelasNice = document.getElementById('kelas_nice_indikasi_geografis').value.trim();
        const sejarah = document.getElementById('sejarah').value.trim();
        const tradisi = document.getElementById('tradisi').value.trim();
        const jumlahLisensi = document.getElementById('jumlah_lisensi_indikasi_geografis').value.trim();
        const deskripsi = document.getElementById('deskripsi').value.trim();
        const tanggalIndikasi = document.getElementById('tanggal_indikasi_geografis').value;

        if (!judul) return alert('Judul harus diisi'), false;
        if (!namaBarang) return alert('Nama Barang harus diisi'), false;
        if (!kualitas) return alert('Kualitas harus diisi'), false;
        if (!karakteristik) return alert('Karakteristik harus diisi'), false;
        if (!kelasNice) return alert('Kelas NICE harus diisi'), false;
        if (!sejarah) return alert('Sejarah harus diisi'), false;
        if (!tradisi) return alert('Tradisi harus diisi'), false;
        if (!jumlahLisensi) return alert('Jumlah Lisensi harus diisi'), false;
        if (!deskripsi) return alert('Deskripsi harus diisi'), false;
        if (!tanggalIndikasi) return alert('Tanggal Indikasi Geografis harus diisi'), false;
        ...
        if (!hasFile) {
            alert('Minimal 1 dokumen harus diupload');
            return false;
        }
    }
